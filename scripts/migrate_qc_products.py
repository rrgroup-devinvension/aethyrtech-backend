...
def migrate_qc_products():
    """Migrate products safely in batches via raw SQL."""
    logger.info("Starting data migration from aethyrtech to xbytesdata...")
    ...
    with connection.cursor() as cursor:
        ...
        if total_rows == 0:
            logger.info("No records found in aethyrtech.qc_products.")
            return

        logger.info(f"Total products to migrate: {total_rows}")
        ...
        while offset < total_rows:
            ...
            offset += batch_size
            logger.info(f"  - Progress: {min(offset, total_rows)} / {total_rows} rows migrated")
