...
from apps.scheduler.utility.xbyte_client import XByteClient
...
class QuickCommerceClient:
    ...
    def fetch_results(keyword, pincode, platform):
        ...
        logger.info(
            f"QuickCommerce fetch → keyword={keyword}, "
            f"pincode={pincode}, platform={platform}"
        )

        try:
            client = XByteClient()

            # CALL CENTRALIZED CLIENT
            response = client.result(
                zipcode=pincode,
                keyword=keyword,
                platform=platform
            )
            # response = QuickCommerceClient._mock_response(
            #     keyword=keyword,
            #     pincode=pincode,
            #     platform=platform
            # )
            ...
            if response.get("results"):
                dd_log_success(pincode=pincode, keywords=[keyword], response={'platform': platform, 'count': len(response.get('results', []))})
                return response
            ...
            message = (
                ...
                or "No products returned from API"
            )

            logger.warning(f"QuickCommerce API logical error → {message}")
            dd_log_error(pincode=pincode, keywords=[keyword], error=message, extra={'response': response})
            ...
            logger.exception("QuickCommerce unexpected error")
            dd_log_error(pincode=pincode, keywords=[keyword], error=str(e), extra={'exception': str(e)})
            raise ExternalAPIException(
                message="QuickCommerce API failure",
                extra=str(e)
            )
    @staticmethod
