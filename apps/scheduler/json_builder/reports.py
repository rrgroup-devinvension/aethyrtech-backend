...
def reports_data_builder(task, brand_id=None, brand_name=None, template='reports', platform_type=None):
    ...
    """
    logger.info(f"Starting REPORTS JSON build for task {getattr(task, 'id', 'unknown')}")
    time.sleep(0.2)
    ...
    try:
        ...
        logger.info(f"Completed QCOMMERCE REPORTS JSON for task {getattr(task, 'id', 'unknown')}")
