...
def build_json(brands, keywords, products, task, brand_id, brand_name, template, platform_type):
    if template == JsonTemplate.BRAND_DASHBOARD.slug:
        ...
        return category_view_data_builder(task, brand_id, brand_name, template, platform_type)
    elif template == JsonTemplate.BRAND_AUDIT.slug:
        return brand_audit_data_builder(task, brand_id, brand_name, template, platform_type)
...
def perform_json_build(task):
    logger.info(f"JSON_BUILD STARTED | Task={task.id}")
    ...
    logger.info(f"JSON_BUILD COMPLETED | Task={task.id}")
