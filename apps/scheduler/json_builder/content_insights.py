...
def content_insights_data_builder(task, brand_id=None, brand_name=None, template='content_insights', platform_type=None):
    ...
    """
    logger.info(f"Starting CONTENT_INSIGHTS JSON build for task {getattr(task, 'id', 'unknown')}")
    time.sleep(0.2)
    ...
    try:
        ...
        logger.info(f"Completed CONTENT_INSIGHTS JSON build for task {getattr(task, 'id', 'unknown')}")
