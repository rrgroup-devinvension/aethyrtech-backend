        ...
        try:
            ...
            for row in csv_reader:
                ...
                try:
                    ...
                    if order_val != '':
                        try:
                            defaults['order'] = int(order_val)
                        except ValueError:
                            defaults['order'] = 0
                    obj, created = CategoryKeyword.objects.get_or_create(category=category, keyword=keyword, platform=platform, defaults=defaults)
                    if not created and defaults.get('order') is not None:
                        # update existing object's order
                        obj.order = defaults.get('order')
                        obj.save()
                    if created:
                        added_count += 1
                    else:
