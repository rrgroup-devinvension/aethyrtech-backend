...
def extract_platform_name(filename):
    """Extract platform name from filename using regex heuristics."""
    ...
    for platform, files in grouped_files.items():
        ...
        if os.path.exists(map_file_path):
            ...
            try:
                ...
                raise Exception(f"Failed to read preview for {platform}: {e}") from e

            prompt = (
                f"You are an expert data engineer analyzing an e-commerce/retail-media "
                f"dataset for the platform '{platform}'.\n"
                f"Below is a snapshot of the first 30 rows of the raw data spreadsheet (as a 2D array):\n"
                f"{json.dumps(raw_rows)}\n\n"
                "INSTRUCTIONS:\n"
                "1. Identify the 'header_row_index' (0-based index) where the actual table headers begin. "
                "(Often rows 0-5 might contain logos, report titles, or metadata. "
                "The header row is the one with columns like 'Date', 'Sales', 'Cost', 'Impressions').\n"
            ...
            response = llm.generate_content([
                {
                    'role': 'system',
                    'content': 'Map varying dataset columns to standardized KPIs and infer correct header index.'
                },
            ...
            try:
                ...
                raise Exception(f"LLM Dynamic Mapping Failed for {platform}: {e}") from e
        ...
        weekly_df = (
            combined_df.resample('W-MON', on='Date', closed='left', label='left')[numeric_cols]
            .sum()
            .reset_index()
        )
        ...
        for w in weekly_records:
            summary_lines.append(
                f"Week {w['Date']} - Rev: {w['Revenue']}, Spend: {w['Spend']}, "
                f"Impr: {w['Impressions']}, Clicks: {w['Clicks']}, Orders: {w['Orders']}"
            )
        summary_text = "\n".join(summary_lines)
