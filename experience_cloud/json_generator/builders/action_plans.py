...
def action_plans_builder(region_data: dict, task, products=None, template="template-name") -> tuple[bool, dict]:
    ...
    if target in ('all', 'experience'):
        ...
        region_id, "insights")
            ...
            logger.info(f"INSIGHTS template not found for brand {{current_brand}}. Skipping experience insights.")
    ...
    try:
        ...
        logger.info(f"RISK_DATA template not found for brand {{current_brand}}. Using empty dashboard context.")
